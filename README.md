...
```
/plugin marketplace add https://github.com/ax-inc-dev/claude-cursor-orchestration.git
/plugin install claude-cursor-orchestration
```

これで Claude Code 側の設定（Hook、Skill、ブリッジ）が全て有効になります。

**Cursor 側の設定も必要な場合：**
```bash
git clone https://github.com/ax-inc-dev/claude-cursor-orchestration.git
cd claude-cursor-orchestration
./setup.sh   # Cursor側の Skills/Agents/Rules を ~/.cursor/ に展開
```

> Plugin インストールで Claude Code 側は完了します。`setup.sh` は Cursor エディタ側の設定（Skills 20個、Agents 7個、Rules）のみ追加で展開します。
