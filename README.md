...
### メソッド1（推奨）: Claude Code プラグインとしてインストール

Claude Code のプラグイン機能で、このリポジトリをマーケットプレイスとして追加し、ワンコマンドでインストールできます。

```
/plugin marketplace add bunta-ishiwata/claude-cursor-orchestration
/plugin install claude-cursor-orchestration
```

これでプラグイン定義（`.claude-plugin/plugin.json`）、フック（`hooks/`）、スキル（`skills/app-development/SKILL.md`）などが Claude Code 側に入ります。

**注意:** Cursor 側の Skills / Agents / Rules（`~/.cursor/` への展開）や `cursor_dispatch.py` を参照するブリッジパスの環境依存部分は、**引き続きリポジトリを clone したうえで `setup.sh` を実行する**必要があります。プラグインだけでは Cursor エディタ側の設定は行われません。

### メソッド2: `setup.sh` によるセットアップ（従来の手順）
